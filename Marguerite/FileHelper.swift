...
class FileHelper: NSObject, SSZipArchiveDelegate {
    ...
    class func tempFolderPath() -> String? {
        ...
        if let url = NSURL(string: MargueriteGTFSDataURL) {
            let task = NSURLSession.sharedSession().dataTaskWithURL(url, completionHandler: { (data: NSData?, response: NSURLResponse?, error: NSError?) -> Void in
                if let data = data where error == nil {
                    ...
                    if let path = FileHelper.tempZipPath() {
    ...
    */
    private func movedNewZipToTempFolder() {
        if let tempFolder = FileHelper.tempFolderPath(), let tempZipPath = FileHelper.tempZipPath() {
    ...
    */
    private func moveTempFilesToTransitFolder() {
        if let tempFolder = FileHelper.tempFolderPath() {
    ...
    */
    class func moveBundleToTempFolder() {
        if let tempFolderPath = tempFolderPath() {
            for fileNames in gtfsFileNames {
                if let path = NSBundle.mainBundle().pathForResource("BundledGTFS", ofType: "zip") {
                    var isDir : ObjCBool = false
                    NSFileManager.defaultManager().copyItemAtPath(path, toPath: tempFolderPath.stringByAppendingPathComponent("temp.zip"), error: nil)
                }
            }
    ...
    */
    ...
    class func documentsPath() -> String? {
        if let url = NSFileManager.defaultManager().containerURLForSecurityApplicationGroupIdentifier("group.edu.stanford.Marguerite") {
